...
class BAW {
    constructor(clientId, clientSecret) {
        console.log("Welcome to BAW script.");
...
async function test() {
    try {
        const sample = new BAW("r9VlPfHk0MEx7Mfw+izyei+E0ZUhfkghqGgv2Zeb+H4=", "ef7ac57cc66a42b5efd695e058d1fdd1AidDbU82jomHcISlVSOwBSAxQvG7KKwuEcWREBBISh0=")
        let token = await sample.token;
        console.log(token)
        console.log(await sample.getStationCount("Hanoi"))
        console.log(await sample.getStationCount("Sapa"))
        await sample.getTrips("2020-08-12", 2, "Hanoi", "Sapa")
        console.log("Number of trips: " + await sample.tripData.length)
        console.log(await sample.creditData)
        await sample.bookCheapestTrip("john", "Doe", "johndoe@gmail.com", "+1 (234) 556-6677")
        if (sample.cheapestData) {
            console.log(await sample.bookingData.reference);
            console.log(await sample.creditData)
            console.log(await sample.payForBooking())
            console.log(await sample.getCredit())

            let id = setTimeout(async function run() {
                console.log("Timeout exceeded but booking not approved");
                console.log(await sample.creditData);
                clearInterval(sid)

            }, 120000);
            let sid = setInterval(async () => {
                if (await sample.getBookingStatus() === "approved") {
            ...
            console.log("no trip matches the criteria")
        }
